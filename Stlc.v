...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_var2 : forall y, x <> y -> substi s x (tm_var y) (tm_var y)
  | s_abs1 : forall y T t1 t2, x <> y -> substi s x t1 t2 -> substi s x (tm_abs y T t1) (tm_abs y T t2)
  | s_abs2 : forall T t1, substi s x (tm_abs x T t1) (tm_abs x T t1)
...
Proof.
  intros s x t t'. split; intro; generalize dependent t'.
  - induction t; intros; simpl in H;
    try (unfold eqb_string in H; 
         destruct (string_dec x s0) in H; 
         subst; auto);
    try (rewrite <- H; auto).
  - intros. induction H; auto.
    + unfold subst. unfold eqb_string.
      destruct (string_dec x x). auto.
      exfalso. auto.
    + unfold subst. unfold eqb_string.
      destruct (string_dec x y0).
      contradiction. auto.
    + apply s_abs1 with (y := y0) (T := T) in H0.
      unfold subst. rewrite <- IHsubsti.
      fold subst. 
      assert (P: eqb_string x y0 = false).
      unfold eqb_string. destruct (string_dec x y0).
      auto. contradiction. auto.
      rewrite P. auto. auto.
    + simpl. unfold eqb_string. destruct (string_dec x x). 
      auto. contradiction.
    + simpl. subst. auto.
    + simpl. subst. auto.
Qed.
...
Proof.
  ...
  apply T_Abs.
  apply T_Abs.
  apply T_App with (T2 := <{ Bool }>). apply T_Var.
  apply update_neq. intros Contra. discriminate.
  apply T_App with (T2 := <{ Bool }>).
  apply T_Var. apply update_neq. intros Contra. discriminate.
  apply T_Var. apply update_eq.
Qed.
...
Proof.
  ...
  induction T2.
  - intro T1. induction T1; intros;
    discriminate H0.
  - intro T1. induction T1; intros;
    inversion H0;
    eapply IHT2_1; apply H1.
Qed.
(** [] *)
