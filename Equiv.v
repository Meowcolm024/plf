...
Proof.
  intros b c1 c2 Hb.
  split; intros H; unfold bequiv in Hb; 
  simpl in Hb.
  - inversion H; rewrite Hb in H5. 
    + discriminate H5.
    + apply H6.
...
Proof.
  ...
  - unfold aequiv in Ha; simpl in Ha.
    assert (Hx : st =[ x := a ]=> (x !-> aeval st a ; st)).
    apply E_Asgn. auto. 
    rewrite <- Ha in Hx.
    rewrite t_update_same in Hx.
    inversion H. subst. apply Hx.
  - inversion H; subst. unfold aequiv in Ha; simpl in Ha.
    rewrite <- Ha in H. rewrite <- Ha.
    rewrite t_update_same.
    apply E_Skip.
Qed.
  (* FILL IN HERE Admitted. *)
