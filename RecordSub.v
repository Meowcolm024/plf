...
Proof with eauto.
  apply S_Arrow.
  auto. apply subtyping_example_1.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  apply S_RcdPerm; auto. apply eqb_neq ...
  apply S_Trans with (U := <{{  "y" : B :: "z" : C :: "x" : A :: nil }}>).
  apply S_RcdDepth ... apply S_RcdPerm ... apply eqb_neq ...
...
Proof with eauto.
  unfold trcd_kj.
  eapply T_App.
  - eapply T_Abs; unfold TRcd_j.
    + auto.
    + apply T_Proj with (T := <{{  "j" : B -> B :: nil }}>) ...
  - unfold TRcd_j.
    apply T_Sub with (S := <{{ "k" : A -> A :: "j" : B -> B :: nil }}>).
    + eapply T_RCons; eauto.
    + apply S_Trans with (U := <{{ "j" : B -> B :: "k" : A -> A :: nil }}>).
      apply S_RcdPerm ... apply eqb_neq ... apply S_RcdDepth ...
Qed.
(** [] *)
...
Lemma top_eq : forall T, <{{ Top }}> <: T -> <{{ Top }}> = T. Admitted.

Lemma absurd: forall S1 S2 T, <{{ S1 -> S2 }}> <: T . Admitted.

Lemma subtr : forall S T U, 
  S <: T -> T <: U -> S <: U.
Proof.
  intros S T U p1. 
  generalize dependent U.
  induction p1.
  - intros U p2. induction p2; auto.
    apply S_Trans with (U := U).
    apply p2_1. apply p2_2.
  - intros U0 p2. induction p2; auto.
    apply S_Trans with (U := S0).
    apply S_Trans with (U := U).
    apply p1_1. apply p1_2.
    apply S_Trans with (U := U0).
    apply p2_1. apply p2_2.
  - intros U p2. apply top_eq in p2.
    rewrite <- p2. apply S_Top. apply H.
  - intros U p2. 
Abort.

Lemma arrow_trans:
  forall S1 S2 T1 T2 U,
  S2 <: S1 ->
  T1 <: T2 ->
  (forall U : ty, S1 <: U -> S2 <: U) ->
  (forall U : ty, T2 <: U -> T1 <: U) ->
  <{{ S2 -> T2 }}> <: U ->
  <{{ S1 -> T1 }}> <: U.
Proof.
  intros S1 S2 T1 T2 U.
  intros H1 H2 H3 H4 H. induction H; auto.
  apply absurd. apply absurd.
  apply S_Arrow.
